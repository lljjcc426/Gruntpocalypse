 ...
 */
open class Hierarchy(val instance: Grunteon) {
    ...
    private fun buildClassInfo(className: String, subClassInfo: ClassInfo?): ClassInfo {
        ...
        if (existInfo != null) {
            ...
            val classNode = instance.workRes.getClassNode(className) ?: ClassInfo.missingClassNode
    ...
    // reference search
    fun checkMissing(classNode: ClassNode): List<ClassInfo> {
        val missingReference = mutableListOf<ClassInfo>()
    ...
    fun checkMissing(methodNode: MethodNode): List<ClassInfo> {
        val missingReference = mutableListOf<ClassInfo>()

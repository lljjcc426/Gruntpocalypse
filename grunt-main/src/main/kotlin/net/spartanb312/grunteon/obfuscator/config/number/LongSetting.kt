...
class LongSetting(
...
) : NumberSetting<Long>(nameMulti, value, range, step, description, visibility) {
    ...
    override fun parseString(string: String) {
        try {
            value = string.toLong().coerceIn(range)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    override val defaultPercentBar: Float
        get() = ((defaultValue - range.start) / (range.endInclusive - range.start).toDouble()).toFloat()

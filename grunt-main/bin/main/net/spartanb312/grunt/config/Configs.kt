...
object Configs {
    ...
    fun applyConfig(jsonObject: JsonObject) {
        configs.forEach {
            jsonObject[it.name]?.asJsonObject?.let { jo -> it.getValue(jo) }
        }
    }
    ...
    fun saveConfig(path: String) {
        ...
        exportConfig().saveToFile(configFile)
    }
